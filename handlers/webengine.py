        ...
        if self.geolocation_handler:
            if message.startswith('定位成功:'):
                try:
                    ...
                    self.geolocation_handler.geolocation_success.emit(lat, lon, accuracy)
                except Exception as e:
                    print(f"[定位] 解析定位结果失败: {e}")
                    import traceback
                    traceback.print_exc()
                    self.geolocation_handler.geolocation_error.emit("定位结果解析失败")
            elif message.startswith('定位失败:'):
                error_msg = message[len('定位失败:'):].strip()
                print(f"[定位] 定位失败: {error_msg}")
                self.geolocation_handler.geolocation_error.emit(error_msg)
